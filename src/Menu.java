    ...
    private void addmenu(){
        switch (category){
            case "Burger" -> {
                ...
                menuItemList.add(new MenuItem("McSpicy Shanghai Burger Meal", 7100, "쌀가루가 더해져 더 바삭해진 100% 닭가슴살 패티가 들어간 버거"));
                menuItemList.add(new MenuItem("McCrispy Deluxe Burger Meal", 7900, "100% 통닭다리살 겉바속촉 케이준 치킨 패티에 브리오쉬 번을 사용한 버거"));
                ...
                menuItemList.add(new MenuItem("Double Quarter Pounder with Cheese Meal", 9100, "두 배 이상 커진 1/4 파운드 비프 패티가 두 장! 치즈 두 장이 입안 가득 완벽하게 조화되는 놀라운 맛"));

            }
            case "Side" -> {
                menuItemList.add(new MenuItem("Shrimp Snack Wrap", 6300, "통통한 새우살 가득한 슈림프 패티에 화이트마요와 스위트 칠리 소스로 매콤달콤하게!"));
                menuItemList.add(new MenuItem("Shanghai Chicken Snack Wrap", 6300, "매콤한 치킨에 달콤한 화이트 마요 소스로 매콤달콤하게!"));
                ...
                menuItemList.add(new MenuItem("McSpicy Chicken Tenders", 8200, "부드러운 100% 닭안심살을 스파이시 시즈닝으로 매콤 바삭하게 튀겨낸 치킨 텐더!"));
                menuItemList.add(new MenuItem("McNuggets", 8600, "바삭하고 촉촉한 치킨이 한 입에 쏙! 다양한 소스로 입맛에 맞게 즐겨보세요! 4조각"));
                menuItemList.add(new MenuItem("Golden Mozzarella Cheese Sticks", 8600, "속이 꽉 찬 황금빛 바삭함! 자연 모짜렐라 치즈로 빈틈 없이 고소한 치즈스틱 2조각"));

            }
            case "Dessert" -> {
                menuItemList.add(new MenuItem("Bite Sized Choco Churros", 6300, "바삭한 초코 츄러스에 가득 찬 진한 초코 필링!"));
                menuItemList.add(new MenuItem("Oreo McFlurry", 6300, "우유 듬뿍 신선한 아이스크림에 아삭아삭 오레오 쿠키가 가득!"));
                menuItemList.add(new MenuItem("Strawberry Oreo McFlurry", 7500, "상큼한 딸기와 달콤한 오레오 쿠키가 신선한 우유 아이스크림에 퐁당!"));
